...
class Intermediate(nn.Module):
    ...
    def __init__(self, input_features, middle_features, num_conv_layers):
        ...
        for i in range(len(part)):
            ...
            if part[i] == 5:
                ...
                layers_5.append(nn.Conv2d(in_channels=self.middle_features, out_channels=self.middle_features, kernel_size=self.kernel_size, padding=self.padding, bias=False))
                layers_5.append(nn.BatchNorm2d(self.middle_features))
                layers_5.append(nn.ReLU(inplace=True))
                ...
                layers_5.append(nn.Conv2d(in_channels=self.middle_features, out_channels=self.output_features, kernel_size=self.kernel_size, padding=self.padding, bias=False))

                self.itermediate_5 = nn.Sequential(*layers_5)
    ...
    def forward(self, x):
        ...
        x4 = self.down(x3)
        x5 = self.itermediate_3(x4)
        x6_0 = self.up(x5, x3)
        x6 = self.itermediate_4(x6_0)
        x7 = self.up(x6, x1)
        out = self.itermediate_5(x7)
        return out
