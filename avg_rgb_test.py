...
parser.add_argument("--input", type=str, default="Kodak24", help='path to input image ')
...
def main():
    print('Loading model ...\n')
    # Create model
    net = FEUNet(num_input_channels=3)
    device_ids = [0]
    model = nn.DataParallel(net, device_ids=device_ids).cuda()
    model.load_state_dict(torch.load(os.path.join('models', 'net_rgb.pth')))
    model.eval()
